export default class AudioPlayer {
	...
	#isPlaying;
	#dragType;
	#volumeBeforeMute; // Declare #volumeBeforeMute as a private field
	...
	#setTime = (e) => {
		...
		const progressPercentage =
			(clickPositionInBar / this.#progressBar.offsetWidth) * 100;
		this.#audio.currentTime = (progressPercentage / 100) * this.#audio.duration;
	};

	#setVolume = (e) => {
		const clickPositionInBar =
			e.clientX - this.#volumeSlider.getBoundingClientRect().left;
		const volumePercentage =
			(clickPositionInBar / this.#volumeSlider.offsetWidth) * 100;
	...
	#handleKeyDown(e) {
		...
		if (e.keyCode === ARROW_LEFT || e.keyCode === ARROW_RIGHT) {
			if (e.currentTarget === this.#progressBar) {
				...
				let newVolume =
					this.#audio.volume + (e.keyCode === ARROW_LEFT ? -0.05 : 0.05);
	...
	init() {
		...
		this.#progressThumb.addEventListener('mousedown', (e) =>
			this.#startDrag(e, 'progress')
		);
		this.#volumeThumb.addEventListener('mousedown', (e) =>
			this.#startDrag(e, 'volume')
		);
	}
}